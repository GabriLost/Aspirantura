...
public class ReleaseUpdateStrategyTest {
    ...
    public void setup() {
        ...
        releaseUpdateStrategy = new ReleaseUpdateStrategy(mockVersionService);
    }
    ...
    public void testImportRelease_invalidRelease_importErrorResult() throws Exception {
        ...
        when(mockVersionService.validateUpdate(mockUser, mockReleaseBuilder)).thenReturn(validationResult);

        ImportResult importResult = releaseUpdateStrategy.importRelease(mockUser, mockReleaseBuilder, null, "project");
    ...
    @Test
    public void testImportRelease_validRelease_updateErrorResult() throws Exception {
        ErrorCollection errorCollection = mock(ErrorCollection.class);
        when(errorCollection.toString()).thenReturn("update error");
        VersionService.VersionBuilderValidationResult validationResult = mock(VersionService.VersionBuilderValidationResult.class);
        ...
        when(mockVersionService.validateUpdate(mockUser, mockReleaseBuilder)).thenReturn(validationResult);
        when(mockVersionService.update(mockUser, validationResult)).thenReturn(serviceOutcome);

        ImportResult importResult = releaseUpdateStrategy.importRelease(mockUser, mockReleaseBuilder, null, "project");

        assertEquals(ResultType.RELEASE, importResult.getType());
        assertEquals(ResultState.ERROR, importResult.getState());
        assertEquals("update error", importResult.getValue());
    }
    ...
    public void testImportRelease_validRelease_updateStatusErrorResult() throws Exception {
        ...
        ImportResult importResult = releaseUpdateStrategy.importRelease(mockUser, mockReleaseBuilder, "closed", "project");
